...
class RecurringPaymentLine(models.Model):
    ...
    state = fields.Selection(selection=[
        ('draft', 'Draft'),
        ('done', 'Done'),
        ('cancel', 'Cancelled')  # ✅ Add this line
    ...
    def action_create_payment(self):
        ...
        if payment:
            ...
            self.write({
                ...
                'journal_entry_id': payment.move_id.id  # ✅ linking journal entry
            })

    def action_revert_payment(self):
        for line in self:
            if not line.payment_id:
                raise ValidationError(_('No payment found to revert.'))

            move = line.payment_id.move_id
            if not move:
                raise ValidationError(_('No journal entry linked to this payment.'))

            if move.state == 'posted':
                move.button_draft()

            move.unlink()  # Delete journal entry
            line.payment_id.unlink()  # Delete payment
